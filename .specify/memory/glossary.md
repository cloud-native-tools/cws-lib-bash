...
|-----------|----------|---------|--------|--------|
| cws_bash_env | - | Entry script (bin/) that loads the whole library into the current shell via source | auto | proposed |
| cws_bash_run | - | Entry script (bin/) that executes a single library function one-shot: cws_bash_run <fn> [args] | auto | proposed |
| Universal Generality | - | Constitution Principle I: every function must be general-purpose with zero project-specific logic | auto | proposed |
